...
async function triggerCalculation() {
    ...
    try {
        ...
        // Sichere Verwendung der formData Werte mit Fallbacks für die Berechnung
        const heatedAreaCalc = appStore.formData.heatedArea ?? 100; // Fallback auf 100m²
        const constructionYearCalc = appStore.formData.constructionYear ?? 2000; // Fallback auf Jahr 2000

        // Simuliertes Ergebnis
        const dummyResult = {
            heizlastGesamt: Math.round(heatedAreaCalc * 75 * (constructionYearCalc < 1980 ? 1.2 : 1)),
...
/* (FontAwesome muss importiert sein in main.ts) */
.section {
  padding-top: 2rem;
  padding-bottom: 4rem;
  min-height: 100vh;
